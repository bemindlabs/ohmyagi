...
### A CLI that builds AGI agents you actually own

*Capability that stays with the person, not the organization.*

[![Status](https://img.shields.io/badge/status-v0.4.1-green)](.scrum/backlog.md)
...
[![Decisions](https://img.shields.io/badge/decisions-D--001%20→%20D--069-informational)](.scrum/decisions.md)
...
> **v0.4.1.** The full MVP is met (since v0.1.0): identity, isolation, a git repository per
...
**Oh My AGI** (`ohmyagi`) is a command-line tool that creates **AGI agents** — as many as you want — where every agent:
...
v0.4.1. 10 epics · 43 stories · 4 spikes · 69 recorded decisions.
The full MVP is met (`.scrum/backlog.md` §7) as of v0.1.0: Phases A and B, the observer, the autonomy core and
recall (E4). v0.2.0 added scheduled triggers (S5.3), the `ohmyagi` name, `ohmyagi setup` and a macOS installer; v0.3.0 the pattern miner (S3.3); v0.4.0 adds agent-to-agent over A2A (E8), a chat connector for Telegram (E9), the web page, the local egress judge, proposal triage, the interest tracker (S3.4) and `ohmyagi update`; v0.4.1 grows the web page (Settings, Agent, Memories, the mascot, https behind tailscale serve, a key that survives restarts). Next: identity inheritance (E6).
